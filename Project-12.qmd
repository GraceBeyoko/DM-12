...
young_pop <- young_pop |> mutate("Value"= Value * 10^3, "Indicator"= "Total population (number)")
```
...
This total young population table has `r population |> ncol()` columns, which are `r population |> names()`. It has `r population|> nrow()` rows and each of them represents a particular country at a particular year. Here is a table with basic statistics of the total population variable. 
```{r}
young_pop |> 
  group_by(Country) |> 
  summarise("min" = min(Value, na.rm=TRUE), "median" = median(Value, na.rm=TRUE), "max"= max(Value, na.rm=TRUE), "mean"= mean(Value, na.rm=TRUE)) |> 
...
```
On créée un nouveau taux d'expenditure

```{r}
graph2_data |> group_by(group) |> ggplot(aes(x= Time, y= illiterate_rate)) + geom_col() + ylab("Number of illiterate people per group") + ggtitle("Distribution of Illiteracy (rate) over time") + facet_wrap(~group)
```
Voir d'où provient le problème: ce n'est pas normal que le taux soit plus grand que 1
