...
PressFreedomIndex <- PressFreedomIndex |>
  pivot_longer(!(Indicator | Country | LOCATION)) |>
  rename("Time" = "name", "Press Freedom Index" = "value") |>
...
```{r}
inner_join(illiteracy_regions, young_pop_regions, by = c("Region", "Time")) |> mutate("rate"= total_value.x/total_value.y) |> ggplot(aes(x= Time, y=rate)) + geom_col() + ylab("Number of illiterate people per region") + ggtitle("Distribution of Illiteracy (rate) over time") + facet_wrap(~Region)
```
...
```{r}
expenditure_rate <- expenditure_rate |> select(Country, Time, -Indicator.x, exp_rate)
...
```{r}
data_inter |> filter(GROUP != 2) |> ggplot(aes(fill=as.factor(GROUP), y=exp_rate, x=as.factor(Time))) + geom_boxplot(position="dodge", alpha=0.5) + coord_flip()+ guides(fill = guide_legend(title = "Group")) + labs(title = "Boxplot of Expenditure on Education Rate Over Time per group",x = "Time", y = "Expenditure on Education Rate") 
```
...
```
## illiteracy vs. enrolment
```{r}
illiteracy_enrol <- inner_join(illiterate_rate, enrolment, by=c("Time", "Country"))
```
```{r}
library(scatterplot3d)
...
s3d <- scatterplot3d(x = illiteracy_enrol$Time, y = illiteracy_enrol$illiterate_rate, z = illiteracy_enrol$Value, 
              color = color_palette[as.numeric(illiteracy_enrol$GROUP)], pch = 16, angle = 130,
              xlab = "Time", ylab = "Illiterate rate", zlab = "Enrollment",
              main = "3D Scatter Plot with Time, Illiterate rate, and Enrollment")

legend(c(7, 12, 30), legend = levels(illiteracy_enrol$GROUP),
       col = c("#999999", "#E69F00", "#56B4E9"), pch = 16, title = "GROUP")
```
```{r}
scatterplot3d(x = illiteracy_enrol$Time, y = illiteracy_enrol$illiterate_rate, z = illiteracy_enrol$Value, color = color_palette[as.factor(illiteracy_enrol$GROUP)], pch = 16, angle = 130, xlab = "Time", ylab = "Illiteracy rate", zlab = "Value", main = "3D Scatter Plot with Time, Illiteracy rate, and Value")

legend("topright", legend = levels(illiteracy_enrol$GROUP), col = color_palette, pch = 16, title = "GROUP")
```
```{r}
color_palette <- brewer.pal(5, "Set3")

scatterplot3d(x = illiteracy_enrol$Time, y = illiteracy_enrol$illiterate_rate, z = illiteracy_enrol$Value, 
              color = color_palette[as.numeric(illiteracy_enrol$GROUP)], pch = 16, angle = 130,
              xlab = "Time", ylab = "Illiterate rate ", zlab = "Enrolement ",
              main = "3D Scatter Plot with Time, Illiterate rate, and Enrolement")
legend("topright", legend = levels(illiteracy_enrol$GROUP), col = color_palette, pch = 16, title = "GROUP")
```

```{r}
illiteracy_enrol |> filter(Time!=2021, !is.na(GROUP)) |> ggplot(aes(x = Value, y = illiterate_rate, label = Country)) + geom_point(alpha = .6) + facet_wrap(~GROUP) + xlab("enrolment percentage") + ylab("illiterate percentage") + ggtitle("impact of enrolment on illiteracy per group")
```
...
```
```{r}
inner_join(illiterate_rate, electricity, by = c("Time", "Country")) |>  inner_join(rural, by = c("Time", "Country")) |> inner_join(GDP_capita, by = c("Time", "Country")) |> filter(Time == 2021) |> 
ggplot(aes(x = `GDP per capita`, y = illiterate_rate,size =rural_per, color = electricity_per)) +
geom_point(alpha = .3) + scale_color_gradient(low = "green", high = "red") + theme_minimal(base_size = 14)
