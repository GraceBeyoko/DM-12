...
```{r}
illiteracy |> filter(Country=="World") |> ggplot(aes(x= Time, y=Value/10^3)) + geom_line() + ylab("Number of illiterate people around the world") + ggtitle("Distribution of Illiteracy (in thousands) over time")
```
...
```
On doit faire un test statistique doit prendre en compte le fait qu'on a des variables temporelles. 
