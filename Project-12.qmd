...
We see that in the educ_expenditure dataset, the column NATMON_IND contains the same value each time. So, we will also remove this column.
```{r, echo=FALSE, results='hide'}
educ_expenditure |>
  distinct(NATMON_IND) |> 
  knitr::kable()
...
We will now apply a data cleaning function to all of the UNESCO data.
```{r, echo=FALSE}
...
We change the units of the young pop dataset and the GDP dataset to millions in order for them to match the other variables. 
```{r, echo=FALSE}
...
We now are going to rename the columns of each datasets for them to match the variables from the UNESCO.
```{r, echo=FALSE}
electricity <- electricity |> 
...
The GDP is expressed in millions US dollars and GDP per capita is expressed in US dollars.
...
This total young population table has `r young_pop |> ncol()` columns, which are `r young_pop |> names()`. It has `r young_pop|> nrow()` rows and each of them represents a particular country at a particular year. Here is a table with basic statistics of the young population variable. 
...
The total net enrolment rate  (primary school) of both sexes is expressed in percentage. 
...
This Rural population share table has `r rural |> ncol()` columns, which are `r rural |> names()`. It has `r rural |> nrow()` rows and each of them represents a particular country at a particular year. Here is a table with basic statistics of the Rural population share variable.
```{r, echo=FALSE, warning= FALSE}
rural |> 
  group_by(Country) |> 
  summarise("min" = min(Value, na.rm=TRUE), "median" = median(Value, na.rm=TRUE), "max"= max(Value, na.rm=TRUE), "mean"= round(mean(Value, na.rm=TRUE),2)) |> 
...
## Graphical Representation (we need to add group 2 to match the other representation)
```{r, echo=FALSE}
...
#Creation of groups of countries based on group of Press Freedom Index (given by Reporters Sans Frontieres)
PressFreedomIndex <- PressFreedomIndex |>
  group_by(Time) |>
