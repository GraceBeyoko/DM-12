...
## illiteracy vs electricity vs rural (we need to specify that the bars means groups)
```{r, echo=FALSE}
...
## illiteracy vs PressFreedom vs Institutions (add legend to specify that color = Group_press so we can remove that y-axis + rename the axis)
```{r, echo=FALSE}
PressFreedomIndex <- PressFreedomIndex |>
  group_by(Time) |>
