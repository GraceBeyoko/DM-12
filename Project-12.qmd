...
```{r}
educ_expenditure |> distinct(NATMON_IND)
```
...
```{r}
electricity |> distinct(`Series Code`, `Series Name`)
```
...
```{r}
PressFreedom |> distinct(`Subindicator Type`, `Indicator ID`)
```
...
institution <- institution |> 
  rename("Country" = "Entity", "LOCATION"= "Code", "Value"= "stability_dem_inst_bti", "Indicator"= "Stability of Institutions")
```
